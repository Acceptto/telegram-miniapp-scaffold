...
router.post('/miniApp/init', async (request, app) => {
	...
	try {
		const hash = request.headers.get('X-Telegram-Init-Data');
		console.log('hashOnBack:', hash);
		console.log('initData from backend', initData);
		...
		console.error('Error in /miniApp/init:', error);
		return new Response('Internal Server Error', { status: 500, headers: { ...app.corsHeaders } });
	}
});
